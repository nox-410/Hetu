...
from train_torch_utils import Config, train, validate, torch_sync_data
...
if __name__ == "__main__":
    ...
    for i in range(args.epochs):
        ...
        if dist.get_rank() == 0:
            print(i, "TRAIN loss {:.4f} acc {:.4f} lr {:.2e}, time {:.4f}".format(
        ...
        acc =acc.mean().item()
        if dist.get_rank() == 0:
            print(i, "EVAL  loss {:.4f} acc {:.4f}".format(loss, acc))
